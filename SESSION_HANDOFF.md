...
- **The user asked about "going live" this session.** V1's live production site (domain seen during testing: `gctalkpicker.app`) has **no deployment config anywhere in the repo** — no Netlify/Vercel config, no GitHub Actions, nothing; V1's own README only documents running it locally via `python3 -m http.server`. Whatever actually deploys `gctalkpicker.app` is external to this repo and wasn't identified this session. Don't assume a deploy mechanism exists or guess at one — ask the user directly if this comes up again. Also worth knowing: V2 is currently missing real V1 features (Draw-specific filters by year/conference/speaker, an "undo last draw," native sign-in) — going live with V2 today would be a regression for real users until those are addressed.
